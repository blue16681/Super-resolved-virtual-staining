...
# from improved_diffusion.unet import CompressCNN_van as CompressCNN
    ...
    for iter in range(0, 1):
        ...
        if dist.get_rank() == 0:
            ...
            for i in range(args.num_samples):
                ...
                np.save(os.path.join(save_dir, files[i].replace('.mat','_af.npy')), af_img)
...
def create_argparser(avg_steps=0):
    defaults = dict(
        ...
        log_dir="log",
        save_dir="outputs_5x_mean",
        use_fp16=False,
        fp16_scale_growth=1e-3,
    )
    if defaults['ckpt'] is not None:
        defaults['save_dir'] = os.path.join(defaults['save_dir'], defaults['model_name'], str(defaults['timestep_respacing'])+'_'+str(defaults['avg_steps']))
...
if __name__ == "__main__":
    ...
    avg_step_list = [50]
    for avg_step in avg_step_list:
        args = create_argparser(avg_step).parse_args()
        main(args)
